...
class GoFish:
    ...
    def __init__(self, ranks: int, suit_size: int, initial_hand_size: int, player_ais: list[Player]) -> None:
        ...
        # Track completed books for each player
        self.completed_books = [[] for _ in players]
    ...
    def draw(self, player_i, player_you) -> None:
        ...
        # You learns that I have a new card in my hand (but you don't know which one)
    ...
    def check_for_books(self, player_i: int) -> float:
        """
        Checks for completed books and returns the reward.

        Args:
            player_i (int): The player index.

        Returns:
            float: The reward for completing books.
        """
        reward = 0.0
        for rank in range(self.ranks):
            if self.hands[player_i][rank] == 4 and rank not in self.completed_books[player_i]:
                self.completed_books[player_i].append(rank)
                reward += 0.1  # Reward for completing a book
        return reward
    ...
    def play_turn(self) -> None:
        ...
        reward = 0.0  # Initialize immediate reward

        actions = self.possible_actions(player_i)
        if len(actions) == 0:
            l.debug("Go fish! (forced action)")
            self.fish_card(player_i, player_you)
            reward -= 0.01  # Penalty for drawing from the deck
            reward += self.check_for_books(player_i)
            # After updating the reward, call 'update' method on the player
            if hasattr(self.player_ais[player_i], 'update'):
                self.player_ais[player_i].update(self, player_i, reward)
            return
        ...
        if they_should_give_cards:
            ...
            reward += 0.1  # Correctly guessed the opponent's cards
            reward += self.check_for_books(player_i)
        else:
            reward -= 0.05  # Incorrectly guessed the opponent's cards
            if len(self.deck) > 0:
                l.info("Go fish!")
                self.fish_card(player_i, player_you)
                reward -= 0.01  # Penalty for drawing from the deck
